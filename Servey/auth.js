...
// Page load hote hi URL ka referral code pakad kar localStorage mein fix karna
window.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const refFromUrl = urlParams.get('ref');
    if (refFromUrl) {
        localStorage.setItem('referredBy', refFromUrl);
        localStorage.setItem('signup_ref_by', refFromUrl);
        console.log("🔗 URL se Referral Code Capture ho gaya:", refFromUrl);
    }
});
...
async function handleSignup() {
    ...
    // Sabhi jagah se referral code check karna taaki miss na ho
    const urlParams = new URLSearchParams(window.location.search);
    const referredBy = urlParams.get('ref') || localStorage.getItem('referredBy') || localStorage.getItem('signup_ref_by') || sessionStorage.getItem('referredBy') || 'direct';
    ...
    try {
        ...
        if (referredBy && referredBy !== 'direct') {
            ...
            if (refError) {
                ...
                alert("⚠️ Signup successful, but referral table insert failed: " + refError.message);
            } else {
